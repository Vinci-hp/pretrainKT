...
def main():
    ...
    parser.add_argument('-out_dim', type=int, default=12372)
    parser.add_argument('-n_skill', type=int, default=1901)
    parser.add_argument('-skill_size', type=int, default=188)
    ...
    start = time.time()
    # =============data====================
    train_data, test_data, skill_full = getData('ednet')
    ...
    print('*' * 30, '训练模型加载中......', '*' * 30)
    # ==========load pretrain==========================
    pretrain = BERTModel(q_size=opt.out_dim, s_size=opt.skill_size, d_k=16, d_v=16, d_model=128, d_inner=512,
                    n_layers=2, n_head=8, dropout=0.2).to(device)
    checkpoint = torch.load('../pretrain_chkpt/ednet-256.chkpt')
    ...
    train(rnn, pretrain, training_data, testing_data, optimizer, device, opt, 'ednet')
