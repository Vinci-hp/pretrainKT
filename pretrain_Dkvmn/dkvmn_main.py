...
def main():
    ...
    if dataset == 'assist2009_updated':
        ...
        parser.add_argument('--n_question', type=int, default=16891,
        ...
        parser.add_argument('--qa_embed_dim', type=int, default=128, help='answer and question embedding dimensions')
        parser.add_argument('--memory_size', type=int, default=20, help='memory size')
        parser.add_argument('--n_question', type=int, default=50983,
                            help='the number of unique questions in the dataset')
        ...
        parser.add_argument('--qa_embed_dim', type=int, default=128, help='answer and question embedding dimensions')
        parser.add_argument('--memory_size', type=int, default=20, help='memory size')
        parser.add_argument('--n_question', type=int, default=12368,
                            help='the number of unique questions in the dataset')
    ...
    print(params)
    # ========xiugai===========
    train_qa_data, train_q_data, skill_full, train_skill, train_cor, valid_qa_data, valid_q_data, test_skill, test_cor = getData('2009', params.n_question, params.seqlen)
    ...
    checkpoint = torch.load('../pretrain_chkpt/2009-128.chkpt')
    pretrain.load_state_dict(checkpoint['model'])

    # pretrain_qa = BERT4Model(q_size=params.n_question, s_size=params.n_skill, d_k=32, d_v=32, d_model=128, d_inner=512, n_layers=1, n_head=4, dropout=0.1).to(device)
    # checkpoint = torch.load('../pretrain_chkpt/2009_qa.chkpt')
    ...
    for idx in range(params.max_iter):
        ...
        if test_auc >= max(test_):
            ...
            with open('./log/log_2009.txt', 'a') as log_tf:
                log_tf.truncate(0)
